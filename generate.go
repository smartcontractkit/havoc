...
func (m *Controller) generate(namespace string, lfd []*ActionablePodInfo, groupLabels []string, npLabels [][]string) (*ChaosSpecs, error) {
	...
	for _, expType := range m.cfg.Havoc.ExperimentTypes {
		...
		case ChaosTypeLatency:
			...
			for _, mfp := range lfd {
				...
				experiments[mfp.PodName] = experiment
			}
			allExperimentsByType[expType] = experiments
